...
export default function RecipeModal() {
  ...
  const handleSimplifyFor = async (minutes) => {
    ...
    INSTRUCTIONS:
    Convert the recipe into ONLY numbered steps. Provide between 8 and 10 steps (choose the number that best fits).
    ...
    Do NOT include any intro text, headings, commentary, or explanations — output ONLY the numbered steps.`;

    const opts = { temperature: 0.3, max_tokens: 1500 };
    try {
  ...
  const handleSubstitute = async () => {
    ...
    // }
    if (!mealDetails || !missing.trim()) {
      setLLMOutput('Please enter a missing ingredient to suggest substitutes.');
      return;
    }
    setLLMOutput('');
    ...
    const prompt = `${base}
      INSTRUCTIONS:
      User is missing "${missing.trim()}".
      Suggest 3 practical substitutes specifically for this recipe.
    ...
    const opts = { temperature: 0.3, max_tokens: 600 };
    try {
      await runLLM(prompt, opts);
    } catch (e) {
      setLLMOutput(`LLM error: ${e.message || e}`);
      ...
      <div className="bg-white rounded-lg max-w-3xl w-full p-6 relative shadow-lg transform transition-transform animate-fade-in">
        ...
        {modalLoading ? (
          ...
          <>
            ...
            <div className="mb-4">
              <h3 className="font-semibold">Instructions (formatted)</h3>
            ...
            <div className="mb-4 mt-2">
              <h3 className="font-semibold">Want to cook faster || get steps here</h3>
              ...
              <div className="mb-4">
                <h3 className="text-xl text-black mb-3 px-2">
                  Forgot something? Search to get alternatives
                </h3>
                ...
                <div className="mb-3 flex gap-2 items-center">
                  ...
                  />
                  <button
                    onClick={() => handleSubstitute(missing)}
              ...
              {llmLoading ? (
                ...
                <pre className="whitespace-pre-wrap bg-gray-50 p-3 rounded text-sm">{llmOutput || 'Use the buttons above to generate stepwise instructions or substitutes.'}</pre>
              )}
            </div>
